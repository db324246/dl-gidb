;(function(root, factory) {
  ...
  class GIDB {
    ...
    // indexdb 版本校验
    validateVersion() {
      if (validateNullOrFalse(this.indexedDB)) throw new Error('您的浏览器不支持 indexedDB, 请使用 chrome 等浏览器')
    }

    constructor ({ databaseName, version }) {
      this.indexedDB = window.indexedDB || window.webkitIndexedDB 
      || window.mozIndexedDB || window.msIndexedDB || null
      this.validateVersion(databaseName)
    ...
    open(databaseName, version) {
      return requestHandler(
        this.indexedDB.open(databaseName, version)
      )
    }
